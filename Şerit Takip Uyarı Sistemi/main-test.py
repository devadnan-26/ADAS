...
def draw_lanes(image, left_fit, right_fit):
    ...
    cv2.polylines(lane_image, left_pts, isClosed=False, color=(0, 255, 0), thickness=5)
    cv2.polylines(lane_image, right_pts, isClosed=False, color=(0, 255, 0), thickness=5)
...
# Load an example image and run the pipeline
image = cv2.imread("road.png")  # Replace with your image path
